...
export const createProject = async ({item, visibility = "private"}: CreateProjectParams): Promise<DesignItem | null | undefined> => {
    if (!PUTER_WORKER_URL) {
        console.warn('PUTER_WORKER_URL is not defined');
        return null;
    }
    ...
    const resolvedSource = hostedSource?.url || item.sourcePath || (isHostedUrl(item.sourceImage) ? item.sourceImage : '');
    ...
    const resolvedRender =
        hostedRender?.url || item.renderedPath || (isHostedUrl(item.renderedImage) ? item.renderedImage : undefined);
    ...
    const payload = {
        ...rest,
        sourceImage: isHostedUrl(item.sourceImage) ? item.sourceImage : '',
        renderedImage: isHostedUrl(item.renderedImage) ? item.renderedImage : '',
...
export const getProjects = async () => {
    if (!PUTER_WORKER_URL) {
        console.warn('PUTER_WORKER_URL is not set, skipping project list fetch.');
        return [];
    }
    try{
